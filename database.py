...
class Log(Base):
    __tablename__ = 'log'
    created = Column(DateTime,nullable=False, server_default=func.now(), primary_key=True)
...
def set_values_from_mail_log(list_of_mail_log : [MailLogClassInfo] ):
    ...
    for value in list_of_mail_log:
        if value.id:
            ...
            addintoMessage(date_string, id=value.id[0], int_id=value.id_self,
                           str_log=value.another_information, status=False)
        else:
            date_string = format_todatetime(value.date + " " + value.time)
            addintoLog(created=date_string, int_id=value.id_self, str_log=value.another_information,
                        address=value.mail_adress)

def select_values_from_tables(name_address: str ):
    name_address = f"'{name_address}'"
    query = text("select * from (select created, str  from log where log.int_id in"
                 " (Select distinct(int_id) from log l where l.address = "+name_address+")) t1"
                 " union all "
